...
router.post('/', requireMinRole('ENGINEER'), async (req, res) => {
  ...
  try {
    const equip = await query(`
      SELECT cal_interval_months FROM dbo.Equipment WHERE equipment_id = @id AND is_active = 1
    ...
    const calDateObj = new Date(cal_date);
    const dueDate = new Date(calDateObj);
    const intervalMonths = equip[0].cal_interval_months || 12;
    ...
    `, [
      ...
      { name: 'cal_date', type: sql.Date,          value: calDateObj },
