...
**Last updated:** 2026-06-30 — **Governance/Forms dev plan authored + all .md reconciled.** NEW `GOVERNANCE-FORMS-DEVPLAN.md` (Phase 14, for Sonnet to build): four cross-cutting requirements — (1) **QM is the default approver** (`ATCA.approvals.defaultApprover()`, final accept gated to QA_MANAGER, MOD-25 default-approver setting); (2) **forms→records** — any filled+signed form (`ATCA.forms`) runs a **3-step sign-off chain** raiser→direct-superior(MOD-18 `reports_to`)→QM, each step a MOD-31 PIN signature; on ACCEPTED it's frozen to an immutable **record** (`ATCA.records.promote`) stored in MOD-37 File Repo with `file_tag:'RECORD'`, **withdrawable** cross-module; (2b) **all approval flows surfaced in MOD-27** via `ATCA.approvalFlows` registry → new "Approval Flow Map" tab with live stage counts; (3) **form-format change control** — "Request for Revision" button beside every `[FM-NNN][v#]` badge → `atca_revisions` register (DRAFT→PENDING_QM→APPROVED→IN_DEVELOPMENT→READY_FOR_DEPLOY→DEPLOYED), QM-approved before deploy; (4) **isolated trial env** — `ATCA.env` TRIAL mode namespaces governance keys under `trial__` (QM/ADMIN only) + `trial`-branch Vercel preview-deploy gate. New console **MOD-39 Forms/Records/Change-Control** (5 tabs). All in localStorage, reuses `atca_pending_approvals`/`ATCA.fileStore`/`atca_form_id_seq`/MOD-31 PIN. ~26.5h, 18 test cases. DEVELOPMENT-STANDARDS.md bumped v1.1 (added Rules 4–7); ROADMAP.md Phase 13 corrected (MOD-37=File Repo, not Semiconductor) + Phase 14 added; README module table + roadmap + doc list updated. Earlier 2026-06-29 — **Multi-entity switcher + Inter-Company Trading BUILT** (`ATCA.entity` ATCA/ATCT/APF pill in every topbar — injects into `.topbar-alerts`, `nav.navbar .ms-auto`, or `#nav-alerts`; `atca_active_entity` key, reload on switch; `mod-interco` module: Interco PO `ICO-PO-YYYY-NNN` lifecycle DRAFT→ISSUED→ACKNOWLEDGED→FULFILLED→CLOSED, Interco DO `ICO-DO-YYYY-NNN` DRAFT→DISPATCHED→RECEIVED, Shared Assets `scope:'ALL'`; home System card + sidebar link; preview_server interco stubs; USER-MANUAL §16–17). Earlier 2026-06-28 — **MOD-37 File Repository BUILT** (central document store for all modules; `ATCA.fileStore` added to `atca-core.js` — `put/get/list/remove/open/summary`; `atca_file_store` localStorage key; Layout B 3-tab module: File Browser (search+filter by module/tag), Upload File, Storage Overview; 11 demo files seeded from MOD-02/04/05/08/11/14/19/24/33/35; sidebar + home card added; preview-verified ✅). Earlier 2026-06-24 — **MOD-14 Inventory major enhancement: lot expiry tracking (lot_expiry on RECEIPT, default_lot_expiry_days per item, Next Lot Exp. column, Lot Expiry tab with filter/Extend/Scrap per lot, KPI card "Lots Expiring / Expired", lot-ok/lot-soon/lot-over badge CSS); LOT_EXTENSION approval flow (proposed expiry + mandatory evidence → atca_pending_approvals; user-dashboard applyLotExtension() updates lot_expiry in-place on matching RECEIPT records); DISPOSAL mandatory lot selection; Item Lots modal (archive icon per row — all lots with COC/extension file links, hasPendingExt/hasPendingScrap dual-check per lot row); Scrap popup (qty/method/ref/notes → DISPOSAL approval); vendor/AVL + lead_time_days per item (drawn from MOD-12); quick receipt removed; _seedDemoLots() seeds 5 APPROVED RECEIPT lots on first load; user-dashboard LOT_EXTENSION TYPE_COLOR/TYPE_ICON/extraDetails/conditional-qty added. Preview-verified 7/7 ✅.** Earlier 2026-06-22 — **MOD-14 Inventory register rebuilt (preview-verified).** New-item popup now: hazardous toggle makes **MSDS/SDS upload mandatory** (save blocked without it) + "Add Document" for TDS/Test Result/Other; every uploaded doc carries an **expiry date defaulting to +1 year**. **Unit** is a dropdown (Liters/Kg/Ton/g/Cubic Meter/Bags/Carboy(25L)); **Location** is a dropdown sourced from an editable **Locations tab**; added **Critical Level** alongside Reorder; **removed Storage Conditions**. Register table gained Critical + Docs columns and per-row **Edit** + **View Documents** buttons (docs viewer opens MSDS/uploads via blob URL, expiry RAG-coloured). RAG computed client-side (stock vs critical/reorder). **Persistence is client-side `localStorage` (key `atca_mod14_v1`)** seeded once from the bundled demo items — the static/preview deploy has no backend; files stored as data URLs. **Real backend persistence (migration cols critical_level/documents/locations + API) is NOT yet done — frontend/demo only.** NOT yet committed/deployed. Earlier 2026-06-21 — **Sidebar single-source-of-truth + Layout C sidebar conversion (preview-verified).** BUG-NAV-02 (HIGH): Layout A pages shipped an abbreviated hardcoded `<nav id="sidebar">` (missing MPT Process etc.) — `ATCA.sidebar.init()` now injects the canonical `_html` into **both** `#sidebar` and `#atca-sidebar` (longest-prefix active-link match), so the side panel is identical on every page; never edit per-page sidebars, edit `ATCA.sidebar._html`. BUG-NAV-03 (HIGH): Layout C pages (MOD-08/13/17/24) had a top navbar and **no** sidebar → the side panel "disappeared" when navigating to them; converted each to `<body>` + `<nav id="atca-sidebar"></nav>` + `<div class="atca-main">` wrapper (closed before the bottom `<script>`) so the injected sidebar appears. BUG-CSS-02 (MED): `.atca-navbar` was referenced in Layout C markup but never defined → the bar rendered with no background and its white text was invisible; added `.atca-navbar` (navy bg + gold bottom border) to atca-erp.css. BUG-NAV-04 (HIGH): navbar-only pages (MOD-11/12/14/16/18/21–23, mod-chat, user-guide) shipped **no** sidebar container at all — base `body{display:flex}` rendered their top navbar as a tall dark left column; `ATCA.sidebar.init()` now **builds the shell** when neither `#sidebar` nor `#atca-sidebar` exists (wraps existing body content in `.atca-main`, prepends a fresh `#atca-sidebar`), so every page that loads atca-core.js gets the canonical sidebar. login.html doesn't load atca-core.js → untouched. Preview-verified MOD-11/12/16 (Operations + Business), user-guide, Internal Chat — sidebar + top navbar render, 0 shell errors. BUG-NAV-05 (HIGH, regression of BUG-NAV-02): MOD-34/35/36 are intentional **standalone** pages (CSS §"Layout A — Standalone module pages") that reuse `id="sidebar"` for their OWN module nav (class `.atca-sidebar`, with `#sidebar .sidebar-link` switchTab JS). The single-source injection hijacked that nav, and the global `#sidebar{position:fixed}` overrode `.atca-sidebar` by id-specificity, collapsing/clipping the layout. Fix: (1) JS only treats `#sidebar` as the app sidebar when it LACKS the `.atca-sidebar` class, and skips shell-building when any `.atca-sidebar` exists → these pages are left untouched; (2) added `#sidebar.atca-sidebar { position:sticky; width:auto; min-width:200px; … }` (id+class beats id) to re-assert the standalone sticky sidebar. Preview-verified MOD-34/35/36 restored (own module nav + content, no clipping); MOD-11 (shell) and MOD-04 (Layout A) unregressed. **Full Chrome sweep of all 36 modules + changelog/bugreport/chat/user-guide passed.** NOTE: MOD-34/35/36 keep their own standalone nav (with Home link), NOT the global app sidebar — by original design. Cache-bust atca-core.js v6→v7 across 51 HTML. Preview-verified: MOD-04/05 (MPT Process now present, active link correct), MOD-08/17/24 (sidebar + navbar render), 0 console errors. NOT yet committed/deployed. Earlier 2026-06-21 — **MOD-07 NCR & CAPA detail pages BUILT** (completes the §8.7 disposition + §10.2 CAPA workflows; previously the dashboard "Open" buttons were toast stubs). NEW `modules/mod07-ncr-capa/ncr-detail.html` (`?id=` deep-link): full NCR record — Identification, Nonconformance & Containment, **§8.7.1 MRB Disposition** (disposition/date/rationale/CAPA-required; stamps `disposition_by`+date on save), Linked CAPAs table + quick "Raise CAPA"; Save→`PUT /mod07/ncr/:id`, Close NCR→`PUT /:id/close` (QA_MANAGER+, blocks close until a disposition is recorded). NEW `capa-detail.html` (`?id=`): full CAPA — Identification + clickable parent-NCR link, **§10.2.1(b)** Root Cause (method/desc), **§10.2.1(c–d)** Corrective/Preventive Actions, **§10.2.1(e)** Effectiveness Review; Save→`PUT /mod07/capa/:id`, Verify & Close→`PUT /:id/close` (QA_MANAGER+, requires `effectiveness_result`≠Pending); live overdue badge from target date. `index.html` Open buttons rewired from toast-stubs to navigate to the detail pages. **Detail pages use a robust fetch:** try `/:id` (real backend) then fall back to list-find via the list endpoint (the demo resolver `ATCA.demo.lookup` is exact-match only — `/mod07/ncr/:id` 404s in demo/preview). Added `.atca-section-head` CSS. Enriched MOD-07 NCR demo stubs (preview_server.py + atca-demo.js kept in sync: `detected_by_name`, part/lot/WO refs, `immediate_action`, `disposition_rationale`/date). Chrome-verified on localhost:3000 (preview server restarted to reload STUBS): both detail pages render, NCR↔CAPA cross-links round-trip, 397d-overdue computed live, 0 console errors. NOT yet committed/deployed. Earlier 2026-06-21 — **Phase 12 BUILT + UI Sweep Bug Fixes + Sidebar & Layout C Fixes**. **Phase 9 BUILT** (Special-Process Compliance Core). **MOD-30 Pyrometry & Heat-Treat** (NADCAP AC7102/AMS2750H): migration 033 (HeatTreatOven seeded from PCM Equipment sheet — 10 ovens ATCT OVEN 4–16 with temp ranges + aerospace-only flags, PyrometryTest TUS/SAT, Thermocouple expiry, vw_OvenStatus RAG); api/mod30 (alerts/summary, ovens, ovens/:id, pyrometry-tests, thermocouples, **routing** aerospace-only gate); frontend mod30-pyrometry (Oven Register · Thermocouples · Routing Check tabs). **MOD-31 Operator Competency & PIN Sign-off** (NAS410/§7.2): migration 034 (Personnel.operator_pin_hash bcrypt, OperatorCompetency operator×process×customer×bay, OperatorSignoff = e-signature log); api/mod31 (competencies, check competency-gate, set-pin, **signoff** = PIN-verified non-repudiable e-signature with competency gate + audit, signoffs log); frontend mod31-operator-competency (Competency Matrix · Sign-off Log · PIN Sign-off tabs). **Electronic Signature realised as the MOD-31 PIN mechanism.** Mounted /mod30+/mod31; home cards + sidebar; preview_server loads all tools/*_demo.json via glob; tools/mod30_demo.json + tools/mod31_demo.json merged into atca-demo.js. Preview-verified (MOD-30 10 ovens/4 TCs/routing 2-of-2, MOD-31 11 competencies/4 sign-offs; 0 mojibake). NOT yet committed/deployed. Earlier 2026-06-17: Forward roadmap authored: **ROADMAP.md** (Phases 9–12, structure-driven from ATC-PCM-001). Phase 9 (Special-Process Compliance Core): MOD-30 Pyrometry & Heat-Treat (AMS2750/AC7102), MOD-31 Operator Competency & PIN, E-Sign. Phase 10 (Capacity & Process Control): MOD-32 Bay Load Scheduler+tank-fit, MOD-33 Spec/Flowdown Frozen-Process, ECN, AAM. Phase 11 (Chemicals/Safety/Escalation): MOD-34 Chemical & Hazmat, Alert Escalation. Phase 12 (Market/Group Scale): MOD-35 Semiconductor Segment, Multi-entity, Template engine. Recommended next: MOD-30. README "Phase 9 Backlog" replaced with roadmap table. Earlier 2026-06-17: PCM + FPI/Electroplating split + Customer Qualification (3 parts, from customer file `ATC-PCM-001 REV A.xlsx`). **Part A:** migration 029 split MOD-06 — added `ChemBath.process_category` ('NDT_FPI'|'ELECTROPLATING') + 18 electroplating bath_types + Bay/max-dim columns (from PCM Tank Capacity) + seed 11 electroplating baths; backend `?category` filter + nested `fpi`/`plating` alerts/summary; frontend two tabs (FPI Process Tanks · Electroplating/Chemical Processing) w/ per-tab KPIs + envelope column. MOD-06 retitled "Chemical Processing (Electroplating) & FPI Tank Control". **Part B (MOD-28 Process Capability Master):** migration 030 (PcmProcess/PcmCapability/PcmRevision/Mod28Sequence); `tools/import_pcm.py` parses the Excel MasterList → 101 capabilities / 22 processes / 31 customers (Boeing BAC5019, Airbus SEP014, P&W SPOP-311, Meggitt EPRO, SIA, Abbott + semiconductor); backend `api/mod28`; frontend `mod28-pcm` (Capability Matrix + Processes cards + Revision History). **Part C (MOD-29 Customer Qualification):** migration 032 (CustomerQualification + QualGap + QualActivity + QualPeriodicAudit + Mod29Sequence); backend `api/mod29` 5-stage state machine Gap Analysis→Close Gap→Qualification→Award→Periodic Audit with guards (no advance w/ open gaps; no award w/o PASS activity; award QA_MANAGER-gated sets cert+validity+next_audit_due); frontend `mod29-qualification` register + 5-stage tracker detail (gaps→NCR, activities, periodic audits). Mounted /mod28+/mod29; home cards + sidebar; preview loads tools/pcm_demo.json + tools/mod29_demo.json, both merged into atca-demo.js. Dev plan: PCM-QUAL-DEVPLAN.md. Preview-verified; NOT yet committed/deployed. Earlier 2026-06-17: Defect remediation ("fix all defect"). Static `href`/`src` scan of all 41 HTML files → found+fixed 3 defect classes: (DEF-NAV-01, HIGH) 18 Layout-A pages had 8 stale Phase-1 sidebar slugs that 404 → repointed (mod08-audit→mod08-audit-management, mod09-customer→mod09-sales-customer-service, mod10-vendor→mod12-purchasing, mod12-traceability→mod27-value-flow, mod13-traveler→mod13-work-order, mod17-mpt→mod17-mpt-process, mod24-coc→mod24-certificate-of-conformance, mod26-config→mod26-maintenance); (DEF-CSS-01, HIGH) 4 Layout-B/C pages (mod08/13/17/24) linked non-existent `/assets/css/atca-core.css` + `/assets/css/bootstrap-icons.min.css` → fixed to `atca-erp.css` + `/assets/fonts/bootstrap-icons.css` (theme+icons now load); (DEF-NAV-02, LOW) deleted orphan duplicate dir `modules/mod06-bath/`. Re-scan: 0 broken refs; targets verified 200 on preview. Added TEST-PLAN §13.7 REF-01..05 static-reference pre-deploy gate + §14 Run 2026-06-17b. NOT yet committed/pushed/deployed. Earlier 2026-06-16: Full-site quality sweep on https://atca-erp.vercel.app (all 30 module pages via same-origin iframe DOM inspection): all load, KPIs populate, tables show demo rows, no mojibake, no blank pages, no stuck default tabs. Found+fixed 2 defects: MOD-10 and MOD-19 demo `alerts/summary` used wrong field names → blank KPI tiles; corrected to match each page's loadKpis (mod10: due_soon/overdue_jobs/incomplete_checklists/failed_test_pieces; mod19: overdue_analyses/due_soon/low_stock/validation_overdue/lab_accred_expiring). Earlier same day: Demo/offline fallback + pagination fix. (1) NEW `assets/js/atca-demo.js` (window.ATCA_DEMO, 104 endpoints ported from preview_server.py STUBS + added mod09/10/19/20 lists). (2) `ATCA.api.request` now falls back to demo data when the backend is unreachable (Vercel static host returns text/plain 404, or preview returns 404) — GET → mapped demo data or `[]` (so tables show "No records found." not a perpetual "Loading…"); writes → soft "Demo mode — changes not saved" toast; one-time bottom banner. Fixes "tabs not working" on the Vercel static site (no backend = every data fetch 404'd). (3) FIXED latent bug: `ATCA.pagination` was never defined though many pages call `ATCA.pagination.render(...)` — the throw was caught by MOD-09's customers handler and overwrote the table with "Error loading customers."; now implemented (Prev/page-info/Next). (4) atca-core.js bumped ?v=4→?v=5 across all 41 HTML + atca-demo.js?v=1 added before it. Earlier 2026-06-16: MOD-27 Value Flow Tracker BUILT (migration 028 grn_id FK on WorkOrder; api/mod27/index.js — alerts/summary, active-grns, flow/:grn_ref; frontend modules/mod27-value-flow/ — 8-stage clickable pipeline diagram + GRN typeahead lookup with live colour-coded stage status; preview-verified: KPIs, typeahead, flow render, node navigation all pass, 0 console errors; mounted in server.js; home card + sidebar nav added; AS9100D §8.5). Dev plan: MOD27-DEVPLAN.md. Earlier: MOD-25 User & Role Management backend BUILT (api/mod25/index.js: GET /users, POST /users, PUT /users/:id, POST /users/:id/reset-password, GET /alerts/summary; ADMIN mutations, QA_MANAGER+ view; mounted in server.js). **All 26 SoR modules 100% complete — no stubs remaining.** Earlier 2026-06-15: MOD-26 System Maintenance Console (SUPERUSER) BUILT — storage/activity-logs/users/password-control/backup + maintenance-mode toggle; ADMIN-gated (preview admin now role ADMIN). **All 26 SoR modules now built (0 inactive home cards).** Earlier same day: MOD-15 KPI Dashboard BUILT (cross-module health roll-up, AS9100D §9.1); fixed latent MOD-20 page crash (missing alerts/summary stub). Earlier 2026-06-15: UX & System QC Protocol run (TEST-PLAN v1.1 §13–14): 31/31 PASS after fixing 2 defects — Internal Chat broken (dead atca-erp.js ref → atca-core.js?v=4) + app-wide mojibake (double-encoded UTF-8 + BOM repaired across 39 files). Earlier 2026-06-15: Usability Pass — home nav rebuilt (all 27 linked, 0 broken), global ⌂ home button (3 layouts), ThreadingHTTPServer fix, cache-bust v=4. Prior (2026-06-14): App Improvement Pass — initPage crash fix, preview no-cache headers, stub field alignment, router seed data
...
| `modules/mod14-inventory/index.html` | MOD-14 | ✅ Done — Inventory Register (RAG, Next Lot Exp. col, vendor sub-line), Stock Movements (all types incl. DISPOSAL w/ mandatory lot), Lot Expiry tab (filter, extend/scrap per lot), Locations, Units. Item Lots modal per row (archive icon). LOT_EXTENSION + DISPOSAL approvals via atca_pending_approvals. _seedDemoLots() seeds 5 RECEIPT lots on first load. Quick receipt removed. |
| `modules/mod37-file-repository/index.html` | MOD-37 | ✅ Done 2026-06-28 — Central file database. 3 tabs: File Browser (search+filter by module/tag/filename), Upload File (manual store to any module/entity), Storage Overview (by-module bars, by-type bars, recent uploads). ATCA.fileStore global API. 11 demo seed files from MOD-02/04/05/08/11/14/19/24/33/35. |
...
- **DB tables:** `InventoryItem` (unique_id, category, unit, location, storage_conditions, hazardous_flag, incompatible_materials, vendor_id, vendor_name, lead_time_days, default_lot_expiry_days), `InventoryMovement` (movement_type RECEIPT/REQUEST/TRANSFER/DISPOSAL, qty, lot_number, lot_expiry, receipt_location, linked_wo_id), `StockCount`
- **Frontend (enhanced 2026-06-24):** 4-tab — Inventory Register (RAG kanban + Next Lot Exp. column + vendor/lead-time sub-line per row), Stock Movements (RECEIPT captures lot no./DO/expiry/COC; DISPOSAL requires mandatory lot selection), Lot Expiry tab (all lots across items, filter by status, Extend + Scrap buttons per lot), Locations/Units. Item Lots modal (archive icon per item — lot table with COC/extension file buttons, storage location, Extend + Scrap per lot). LOT_EXTENSION approval: proposed expiry + mandatory evidence file → atca_pending_approvals; user-dashboard approveItem() updates lot_expiry on RECEIPT record. DISPOSAL approval: lot_number + method + qty → atca_pending_approvals. Quick receipt removed — all stock via Stock Movements tab. Vendor drawn from MOD-12 /mod12/suppliers?status=APPROVED. _seedDemoLots() seeds 5 APPROVED RECEIPT lots (-12d/7d/18d/180d/365d expiry) on first page load.
...
### 2026-06-28 — MOD-37 File Repository (new module, preview-verified)
**Feature:** Central document store for all modules. `ATCA.fileStore` added to `atca-core.js` as a globally available service. Any module calls `ATCA.fileStore.put(meta, dataUrl)` to store a file and receives back a `file_id`; calls `ATCA.fileStore.open(fileId)` to retrieve and display it in a new tab. Storage: `atca_file_store` localStorage key; auto-sequenced IDs (`FILE-NNNN`).
- **Frontend:** `modules/mod37-file-repository/index.html` (Layout B, 3 tabs). **File Browser** — full table of all files across all modules with search (filename/entity ID) + filter by module + filter by file type; view-details modal, open-in-tab, delete. **Upload File** — manually store any file with module/entity/tag classification. **Storage Overview** — file count + size bars by module, by type, recent uploads list.
- **ATCA.fileStore API:** `put(meta, dataUrl)→file_id` | `get(fileId)→record` | `list(filter)→[]` | `remove(fileId)` | `open(fileId)` (opens data URL in new tab via Blob URL) | `summary()→{total, total_bytes, this_month, modules}`.
- **Design pattern:** modules pass `file_id` references in their own records instead of embedding full data URLs. On view, they call `ATCA.fileStore.open(fileId)`. `_seedDemoFiles()` pre-loads 11 realistic files from MOD-02/04/05/08/11/14/19/24/33/35 on first load. `btoa(unescape(encodeURIComponent(html)))` used for Unicode-safe base64 in seed demo URLs.
- **Sidebar + home card:** "File Repository" link added to SYSTEM section in `atca-core.js` and to `index.html` sidebar/card grid.

### 2026-06-24 — MOD-14 Lot Expiry, Vendor/Lead-Time, Item Lots Modal, Scrap Flow
**MOD-14 Inventory Management — major feature enhancement:**
- **Lot Expiry System:** Each item gains `default_lot_expiry_days`; each RECEIPT movement captures `lot_expiry` date + `receipt_location`. Inventory Register shows "Next Lot Exp." column per item. Dedicated "Lot Expiry" tab with filter (All/Expired/Expiring Soon/OK) and sortable table. KPI card "Lots Expiring / Expired" + badge count on tab.
- **Lot Expiry Extension:** Submit with proposed date + mandatory evidence file → `atca_pending_approvals` (type: LOT_EXTENSION). User-dashboard `approveItem()` branches to `applyLotExtension()` which updates `lot_expiry` on all matching RECEIPT approvals. Pending badge shown instead of button.
- **Vendor (AVL) + Lead Time:** `vendor_id/vendor_name/lead_time_days` added to InventoryItem (drawn from MOD-12 `/mod12/suppliers?status=APPROVED`). Vendor name + lead time shown as sub-line in Name column.
- **Quick Receipt removed:** All stock movements via Stock Movements tab only.
- **Item Lots Modal:** Archive icon per inventory row. Shows all lots for item with lot no., DO, received date, qty, storage location, expiry date, days left, status badge, COC/extension file buttons, Extend + Scrap actions.
- **Scrap popup (#scrapLotModal):** Qty, disposal method, optional vendor ref + notes → pushes DISPOSAL approval with `lot_number`. Accessible from Lot Expiry tab and Item Lots modal.
- **DISPOSAL mandatory lot:** Stock Movements DISPOSAL type now requires selecting a lot from dropdown (populated from approved RECEIPTs for the selected item).
- **user-dashboard.html:** LOT_EXTENSION type added to TYPE_COLOR/TYPE_ICON; `qty` display made conditional (LOT_EXTENSION has no qty); `extraDetails()` shows Lot No./Current Expiry/Proposed Expiry/Evidence for LOT_EXTENSION.
- **_seedDemoLots():** Seeds 5 APPROVED RECEIPT lots on first load (expiry at -12d, 7d, 18d, 180d, 365d).

### 2026-06-21 — Sidebar Injection + Layout C Alignment Fix (preview-verified)
**BUG-LAYOUT-01 (HIGH) — Layout C pages (MOD-08, 13, 17, 24) misaligned (navbar + content side-by-side):**
- Root cause: `atca-erp.css` `body { display: flex; }` defaults to `flex-direction: row`, so Layout C pages (class="atca-body") placed their Bootstrap navbar beside content instead of above it.
- Fix: added `body.atca-body { flex-direction: column; }` to `atca-erp.css`.
- Verified: MOD-08 navbar at top (y=0, h=56px), content at y=56px; no more side-by-side.

**BUG-NAV-01 (HIGH) — Sidebar empty on all Layout B pages (FPI, MPT, etc.) — MPT link disappeared when navigating to FPI:**
- Root cause: `#atca-sidebar` div in Layout B pages was always empty — `ATCA.sidebar.init()` only wired a toggle but injected no HTML.
- Fix: `ATCA.sidebar.init()` in `atca-core.js` now injects the full navigation HTML (all 40 module links, 5 sections) into `#atca-sidebar`, marks the current page link `.active`, and wires the mobile toggle.
- Fix: `atca-erp.css` `#sidebar` rules extended to also target `#atca-sidebar` (same visual style).
- Fix: `.atca-main { margin-left: var(--sidebar-width); }` added so content clears the fixed 260px sidebar.
- Verified: MOD-03 FPI Process — sidebar injected (40 links), FPI active, MPT and Audit Management visible.

### 2026-06-21 — Full UI Sweep: Layout A CSS Fix + Demo Data Field Alignment (preview-verified)
**UI sweep** of all 36 modules + 3 utility pages. Bugs identified and fixed:

**BUG-CSS-01 (HIGH) — Layout A topbar invisible / sidebar unstyled (MOD-34, MOD-35, MOD-36):**
- Root cause: `atca-erp.css` `#topbar { background: var(--atca-white) }` (specificity 1,0,0) overrode the Layout A dark topbar, and `.atca-topbar`, `.atca-sidebar`, `.sidebar-link` etc. had no CSS definitions.
- Fix: added `#topbar.atca-topbar { background: var(--atca-navy); position: fixed; ... }` (specificity 1,1,0 wins) + all companion classes (`.atca-sidebar`, `.sidebar-link`, `.sidebar-section`, `.topbar-brand`, `.topbar-module`, `.topbar-user`, `.topbar-divider`) to `atca-erp.css` before the PRINT STYLES section.
- Verified: topbar=`rgb(27,42,74)`, position=`fixed` on MOD-34/35/36.

**BUG-DEMO-01..05 (MEDIUM) — "undefined" cells in tables (MOD-10, MOD-11, MOD-12, MOD-19, MOD-20):**
- Root cause: `atca-demo.js` and `preview_server.py` used different field names than the frontend HTML template strings.
- MOD-10 `/mod10/route-cards`: renamed `route_card_ref→route_ref`, `process_type→job_type`, `steps_done→done_ops`, `steps_total→total_ops`; added `route_id`, `quantity`, `required_by_date`.
- MOD-11 `/mod11/assets`: renamed `asset_name→name`, `asset_type→category`, `next_pm_date→next_pm_due`.
- MOD-12 `/mod12/suppliers`: renamed `status→approval_status`, `accreditation_number→accreditation_ref`; added `avl_scope`, `accreditation_body`.
- MOD-19 `/mod19/analysis-schedules`: renamed `frequency→frequency_type`, `next_due→next_due_date`, `assigned_to_name→responsible_name`; added `analysis_type`, `process_area`, `last_done_date`, `days_to_due`, `external_lab`.
- MOD-20 `/mod20/complaints`: renamed `owner_name→owned_by_name`; added `complaint_type`, `target_close_date`, `has_8d`.
- Also added MOD-19 and MOD-20/complaints endpoints to `preview_server.py` (they were missing — server returned `{}`).
- All fixes applied to both `atca-demo.js` and `preview_server.py`.
- Verified: all five modules render correct data in all columns; no "undefined" remaining.
